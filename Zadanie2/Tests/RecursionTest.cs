...
namespace Tests
{
    ...
    public class RecursionTest
    {
        ...
        public void CorrectFormatterTest()
        {
            ...
            Zadanie2.CorrectFormatter formatter = new CorrectFormatter();
            File.Delete("final.txt");
            Stream stream = File.Open("final.txt", FileMode.Create, FileAccess.ReadWrite);
            formatter.Serialize(stream, c);
            stream.Close();

            Stream streamDeserialize = File.Open("final.txt", FileMode.Open, FileAccess.Read);
            c2 = (C)formatter.Deserialize(streamDeserialize);
            streamDeserialize.Close();



            Assert.AreEqual(c.ClassName, c2.ClassName);
            A a2 = c2.ClassA;
